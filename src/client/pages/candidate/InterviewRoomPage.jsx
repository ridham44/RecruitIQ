...
import { Bot, Camera, CameraOff, Mic, MicOff, Send, PhoneOff, Video, Wifi, WifiOff, AlertTriangle, Volume2 } from 'lucide-react';
...
// what keeps the mic from ever picking up the AI's own voice.
function speak(text) {
...
export default function InterviewRoomPage() {
  ...
  const askQuestion = useCallback(async (nextStage, nextQuestion) => {
    setStage(nextStage);
  ...
  const handleJoin = async () => {
    ...
    setError('');
    try {
      const { token, url, question: firstQuestion, interview, aiName: startAiName, aiTitle: startAiTitle } = await interviewsApi.start(interviewId);
  ...
  const startRecording = () => {
    if (!SpeechRecognitionCtor || aiSpeaking || recording) return;
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      {phase === 'live' && !SpeechRecognitionCtor && (
        <div className="mb-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-700">
          Your browser doesn't support speech-to-text. Type your answer into the box below and hit Send.
        </div>
      )}
            ...
            <div>
              <div className="mb-4 flex items-center gap-3 rounded-lg bg-brand-50 p-4">
                <div className="rounded-full bg-brand-600 p-2">
                  {aiSpeaking ? <Volume2 className="h-4 w-4 animate-pulse text-white" /> : <Bot className="h-4 w-4 text-white" />}
                </div>
                <div className="flex-1">
                  <p className="text-xs font-medium uppercase tracking-wide text-brand-700">
                    {aiName ? `${aiName}${aiTitle ? ` · ${aiTitle}` : ''} — ` : ''}
                ...
                {secondsLeft != null && !aiSpeaking && (
                  <div className="shrink-0 text-lg font-semibold tabular-nums text-brand-700">{secondsLeft}s</div>
                )}
              </div>
              ...
              <textarea
                ...
                placeholder={recording ? 'Listening… speak your answer' : 'Turn on the mic to speak, or type your answer here'}
                rows={4}
                className="mb-3 w-full resize-none rounded-lg border border-slate-300 p-3 text-sm text-slate-800 focus:border-brand-500 focus:outline-none disabled:bg-slate-50"
              />
              ...
              <div className="flex flex-wrap items-center justify-center gap-3">
                ...
                <Button onClick={() => handleSend(false)} disabled={aiSpeaking || sending || !transcript.trim()} loading={sending}>
